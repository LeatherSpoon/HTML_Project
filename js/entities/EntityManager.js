...
export class EntityManager {
  ...
  update(delta, playerPos, collisionCircles = null) {
    ...
    if (this._gracePeriod > 0) {
      ...
      for (const enemy of this.enemies) enemy.update(delta, playerPos, true, collisionCircles);
      return;
    }

    for (const enemy of this.enemies) {
      const triggered = enemy.update(delta, playerPos, false, collisionCircles);
      if (triggered) {
        this._inCombat = true;
        this.onAggro(enemy);
        return;
