...
export class Player {
  ...
  update(keysDown, delta, touchInput = null) {
    ...
    if (this.isGathering) {
      // Gathering runs automatically once started — movement not locked
      this._gatherProgress += delta;
      if (this._gatherProgress >= this._gatherDuration) {
        return; // Gather complete — handled externally via getGatherResult()
      }
      // Fall through: player can move freely while gathering
    }

    const terrain = CONFIG.TERRAIN[this.currentTerrain] || CONFIG.TERRAIN.grass;
    const speed = this.stats.moveSpeed * terrain.speedMult;
